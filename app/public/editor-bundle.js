var LawgptEditor = (() => {
  ...
  var editor_entry_exports = {};
  __export(editor_entry_exports, {
      ...
      return {
        ...
        removeEmptyTextStyle: () => ({ tr: tr2 }) => {
          ...
          tr2.doc.nodesBetween(selection.from, selection.to, (node, pos) => {
            ...
            if (!node.marks.filter((mark) => mark.type === this.type).some((mark) => Object.values(mark.attrs).some((value) => !!value))) tr2.removeMark(pos, pos + node.nodeSize, this.type);
          });
          return true;
        }
      };
  ...
  var PageBreak = Node2.create({
    ...
    addCommands() {
      return {
        ...
        insertPageBreak: () => ({ chain }) => chain().insertContent([{ type: this.name }, { type: "paragraph" }]).run()
      };
    }
  });
  function createEditor(mountEl, options) {
    options = options || {};
    return new Editor({
      element: mountEl,
      extensions: [
